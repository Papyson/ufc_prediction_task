...
// Global pointers for AI assignment
let globalLastUsedAIFolder = "badAI"; // Initialize to force the first session to use goodAI.
let globalGoodAIIndex = 1;   // For goodAI: trial_set_1.csv ... trial_set_250.csv
let globalBadAIIndex = 251;  // For badAI: trial_set_251.csv ... trial_set_500.csv

// New Map to store session metadata (aiFolder and trialSetFile) for each session.
...
const sessionManager = {
  ...
  /**
   * Starts or joins a session.
   * Returns an object with { sessionID, waitingEndTime, mode, aiFolder, trialSetFile }.
   */
  async startSession(clientID) {
    if (!clientID) {
      throw new Error("clientID is required to start a session.");
    ...
    if (waitingSessionDoc) {
      ...
      if (sessionData.participants.length < 3 && elapsed < WAITING_DURATION) {
        ...
        const aiFolder = sessionData.aiFolder;
        const trialSetFile = sessionData.trialSetFile;
        ...
        if (sessionData.participants.length === 3) {
          ...
          return { sessionID: waitingSessionDoc.id, waitingEndTime: now, mode: 'group', aiFolder, trialSetFile };
        }
        return { sessionID: waitingSessionDoc.id, waitingEndTime, mode: 'waiting', aiFolder, trialSetFile };
      } else if (sessionData.participants.length === 2 && elapsed >= WAITING_DURATION) {
        for (const participant of sessionData.participants) {
          const soloSessionID = await createSoloSession(participant);
          this.clientSessions.set(participant, soloSessionID);
        ...
        this.clientSessions.set(clientID, soloSessionID);
        console.log(`Created solo session ${soloSessionID} for ${clientID}`);
        return { sessionID: soloSessionID, waitingEndTime: now, mode: 'solo', aiFolder: sessionData.aiFolder, trialSetFile: sessionData.trialSetFile };
      } else {
        return await createNewWaitingSession(clientID);
      }
    } else {
  ...
  // Helper methods using the sessionMeta Map.
  getAIFolder(sessionID) {
    const meta = sessionMeta.get(sessionID);
    return meta ? meta.aiFolder : null;
  },

  getTrialSetFile(sessionID) {
    const meta = sessionMeta.get(sessionID);
    return meta ? meta.trialSetFile : null;
  }
};
...
async function createNewWaitingSession(clientID) {
  ...
  const now = Date.now();
  const waitingEndTime = now + WAITING_DURATION;
  // Determine AI type based on global pointers.
  let newAIFolder, trialSetFile;
  if (globalLastUsedAIFolder === "goodAI") {
    newAIFolder = "badAI";
    trialSetFile = `trial_set_${globalBadAIIndex}.csv`;
    globalBadAIIndex++;
    if (globalBadAIIndex > 500) { globalBadAIIndex = 251; }
  } else {
    newAIFolder = "goodAI";
    trialSetFile = `trial_set_${globalGoodAIIndex}.csv`;
    globalGoodAIIndex++;
    if (globalGoodAIIndex > 250) { globalGoodAIIndex = 1; }
  }
  globalLastUsedAIFolder = newAIFolder;
  ...
  const sessionData = {
    ...
    mode: 'waiting',
    finishedIDs: [],
    aiFolder: newAIFolder,
    trialSetFile: trialSetFile
  };
  await sessionRef.set(sessionData);
  const sessionID = sessionRef.id;
  sessionManager.clientSessions.set(clientID, sessionID);
  // Store metadata locally in sessionMeta.
  sessionMeta.set(sessionID, { aiFolder: newAIFolder, trialSetFile: trialSetFile });
  console.log(`Created new waiting session ${sessionID} for ${clientID} with ${newAIFolder} and ${trialSetFile}`);
  setTimeout(async () => {
    const sessionSnap = await sessionRef.get();
    if (sessionSnap.exists) {
      const data = sessionSnap.data();
  ...
  return { sessionID: sessionID, waitingEndTime, mode: 'waiting', aiFolder: newAIFolder, trialSetFile: trialSetFile };
}
