...
const preTask = (function() {
  ...
  function init(webSocketInstance) {
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === "sessionStarted") {
        ...
        sessionStorage.setItem("sessionID", data.sessionID);
        sessionInfo = data;
        // Store AI metadata in sessionStorage for later use
        if (data.aiFolder) {
          sessionStorage.setItem("aiFolder", data.aiFolder);
        }
        if (data.trialSetFile) {
          sessionStorage.setItem("trialSetFile", data.trialSetFile);
        }
        // Instruct trialPhase to load the CSV trial data using the provided metadata.
        trialPhase.loadTrialCSV();
        // Now that sessionStarted has arrived, send the survey data.
        sendPreTaskSurveyData();
        if (data.mode === "waiting") {
          // Show waiting room only if session is in waiting mode
          startWaitingRoom(data.waitingEndTime);
  ...
  function submitSurveyAndStartWaiting() {
    ...
    console.log("Pre-task survey captured:", preTaskData);
    // Wait for sessionStarted message before sending survey data.
    ws.send(JSON.stringify({ type: "startSession", clientID: clientID }));
    hideAllScreens();
  }
